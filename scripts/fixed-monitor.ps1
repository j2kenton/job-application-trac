# Fixed File Monitor with Working Git Commits
param(
...
function Commit-Changes {
    ...
    try {
        Write-ColoredOutput "STAGING: Preparing to commit $($Changes.Count) files..." "Info"
        ...
        # Stage each file individually with error checking
        $stagedFiles = @()
        foreach ($change in $Changes) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-ColoredOutput "WARNING: Failed to stage $relativePath" "Warning"
            }
        }
        ...
        $commitMessage = @"
AUTO-COMMIT: $($stagedFiles.Count) files (${confidence}% confidence)
...
Generated by fixed-file-monitor.ps1
"@
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-ColoredOutput "COMMIT MESSAGE: Auto-commit ($($Analysis.Type), ${confidence}% confidence)" "Success"
        } else {
            Write-ColoredOutput "ERROR: Git commit failed (exit code: $LASTEXITCODE)" "Error"
            
            # Check what git status shows
            $gitStatus = git status --porcelain 2>$null
            if ($gitStatus) {
                Write-ColoredOutput "GIT STATUS:" "Info"
                $gitStatus | ForEach-Object { Write-ColoredOutput "  $_" "Info" }
            }
        }
...
function Process-ChangesBatch {
    ...
    if ($AutoCommit -and $analysis.Type -eq "automated" -and $analysis.Confidence -gt 0.75 -and $Changes.Count -le $MAX_AUTO_FILES) {
        ...
        $reason = if ($analysis.Type -ne "automated") { "not detected as automated" }
                 elseif ($analysis.Confidence -le 0.75) { "confidence too low (${confidence}%)" }
                 elseif ($Changes.Count -gt $MAX_AUTO_FILES) { "too many files ($($Changes.Count))" }
                 else { "unknown reason" }
        Write-ColoredOutput "SKIPPED: Auto-commit skipped - $reason" "Warning"
    }
}
...
Write-ColoredOutput "STARTING: Fixed File Monitor with Working Git Commits" "Success"
...
Write-ColoredOutput "CONFIDENCE THRESHOLD: 75%" "Info"
Write-ColoredOutput ""
...
$action = {
    ...
    if ($using:Verbose) {
        ...
        Write-ColoredOutput "DETECTED: $changeType - $relPath" "Info"
    }
}
